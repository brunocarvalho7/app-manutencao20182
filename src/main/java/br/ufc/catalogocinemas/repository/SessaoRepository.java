...
*/
    ...
    List<Sessao> findByFilmeId(int idFilme);

    List<Sessao> findBySalaId(int idSala);

    @Query(value = "select se.* from sessao as se left join sala as sa on(se.sala_id = sa.id)"
    		+ " left join cinema as c on(sa.cinema_id = c.id) where c.cidade = ?1", nativeQuery = true)
